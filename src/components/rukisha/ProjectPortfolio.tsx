            ...
            <div>
              ...
              <p className="text-white/60 text-sm flex items-center gap-2">
                {view === "active"
                  ...
                  : "Reviewing decommissioned project records."}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <TabsList className="bg-transparent border-none">
                <TabsTrigger
                  value="active"
                  className="data-[state=active]:bg-white/10 data-[state=active]:text-[var(--rk-gold)] text-white/60 text-xs"
                >
                  Active
                </TabsTrigger>
                <TabsTrigger
                  value="archived"
                  className="data-[state=archived]:bg-white/10 data-[state=archived]:text-[var(--rk-gold)] text-white/60 text-xs"
                >
                  Archived
                </TabsTrigger>
              </TabsList>
          ...
          {filteredProjects.length === 0 ? (
            <div className="col-span-full py-24 text-center border-2 border-dashed border-white/10 rounded-2xl bg-white/5">
              ...
              <h2 className="text-white/80 font-medium">
                {view === "active" ? "No active projects" : "Archive is empty"}
              </h2>
    ...
    try {
      ...
      toast.success(
        project.isArchived
          ...
          : "Project moved to historical archive",
      );
    } catch (err) {
      console.error(err);
      ...
      >
        <div className="flex items-start justify-between p-6">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2 text-white/40 text-xs">
              ...
              <span>
                {project.isArchived ? "Decommissioned on " : "Updated "}{" "}
        ...
        {project.isArchived && (
          <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="bg-black/80 text-[8px] uppercase tracking-tighter text-white/60 px-2 py-0.5 rounded border border-white/10">
              Archived
            </div>
          </div>
        )}
        ...
        <AlertDialogContent className="bg-[var(--rk-navy)] border-white/10 text-white">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/60">
              ...
              <span className="text-white font-bold">"{project.name}"</span>? This action will
              destroy all mission data, tasks, and stakeholders. It cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
